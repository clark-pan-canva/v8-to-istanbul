...
module.exports = class CovSource {
  ...
  offsetToOriginalRelative (sourceMap, startCol, endCol) {
    ...
    // TODO, figure out why this logic doesn't work well with our codebase
    /*
    ...
    */
    ...
    // TODO using simpler mapping logic seems to get more correct values
    const start = originalPositionFor(sourceMap, {
      line: lines[0].line,
      column: Math.max(0, startCol - lines[0].startCol),
      bias: LEAST_UPPER_BOUND,
    });
    ...
    let end = originalPositionFor(sourceMap, {
      line: lines[lines.length - 1].line,
      column: endCol - lines[lines.length - 1].startCol,
      bias: LEAST_UPPER_BOUND,
    });
